            ...
            <el-tab-pane v-for="item in modelList" :key="item.uuid+item.name" :label="item.name" :name="item.uuid">
                <template #label>
                    <span class="custom-tabs-label">
                        <span>{{ item.name }}</span>
                        <el-icon style="vertical-align: 0.05em;margin-left: 5px;">
                            <edit @click="openModifyModel(item)" />
                        </el-icon>
                    </span>
                </template>

                <AppTextList v-if="tabShowList.includes(item.uuid)" :appUuid="item.uuid" />
            </el-tab-pane>
        </el-tabs>

        <div v-if="modelList.length" class="pos-tool">
            <el-button type="primary" size="small" @click="openAddModel">新增模板 <i class="el-icon-plus"></i></el-button>
        </div>
...
import AppTextModelDialog from './pages/AppTextModel/EditModel.vue';
import AppTextList from './pages/ApptextList/index.vue';
