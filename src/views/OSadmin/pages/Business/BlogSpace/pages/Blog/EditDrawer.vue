<template>
    <el-drawer size="35%"
        :model-value="visible"
        :title="formModelParams._title || '编辑博文信息'"
        @open="openEvent"
        @close="closeEvent"
    >
        <el-form ref="form" :model="formModel" :rules="formModelRule" label-width="80px" label-position="left">
            ...
            <el-form-item label="博文分组" prop="classify">
...
const { formModelParams, blogclassTree } = toRefs(props);
...
function addFormModel() {
    ...
    proxy.$refs.form.validate(async (valid) => {
        if (valid) {
            ...
            const { code, message } = await service.Add_blog({
