<template>
    <el-drawer size="35%"
        :model-value="visible"
        :title="formModelParams._title || '编辑章节分组信息'"
        @open="openEvent"
        @close="closeEvent"
    >
        <el-form ref="form" :model="formModel" :rules="formModelRule" label-width="80px" label-position="left">
            ...
            <el-form-item label="分组类型" prop="type">
                <el-select size="small" :popper-class="storeProfilesgetter" placeholder="请选择" v-model="formModel.type">
                    <el-option label="世界观" :value="0"></el-option>
                    <el-option label="书籍" :value="1"></el-option>
                    <el-option label="章节分组" :value="2"></el-option>
                </el-select>
            </el-form-item>
...
function addFormModel() {
    ...
    proxy.$refs.form.validate(async (valid) => {
        if (valid) {
            ...
            const { code, message } = await service.Add_bookclass({
...
function modifyFormModel() {
    ...
    proxy.$refs.form.validate(async (valid) => {
        if (valid) {
            ...
            const { code, message } = await service.Modify_bookclass({
                userName: userName.value,
                userPassword: userPassword.value,
...
async function deleteFormModel(row) {
    ...
    if (confirm("此操作不可逆，确认删除？")) {
        const { code, message } = await service.Delete_bookclass({
            userName: userName.value,
            userPassword: userPassword.value,
            _id: formModelParams.value._id,
            id: formModelParams.value.id,
...
function openEvent() {
    ...
    if (formModelParams.value._action === 'modify') {
        Object.keys(formModel).forEach(key => {
            if (![null, undefined, ''].includes(formModelParams.value[key])) {
