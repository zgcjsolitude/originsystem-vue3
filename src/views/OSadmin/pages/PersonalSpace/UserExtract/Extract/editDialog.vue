<template>
    ...
    >   
        <div class="OSA-form">
            <div class="form-input">
                ...
                <el-select v-model="formModel.Sign" :popper-class="storeProfilesgetter" size="small" placeholder="请选择类别">
                    <el-option v-for="(item, i) in vuexUserExtractTypeList" :key="i" :label="item.Value" :value="item.Value"></el-option>
                </el-select>
            </div>
            <div class="form-input">
        ...
        <template #footer>  
            <el-button size="small" type="primary" :loading="editBtnLoading" @click="formModelParams._action == 'add' ? addExtract() : modifyExtract()">
...
const vuexUserExtractTypeList = computed(() => store.state.OsAdmin.vuexUserExtractTypeList);
...
async function addExtract() {
    if (!formModel.Sign) {
        return ElMessage.warning("请建立分组");
    }
    editBtnLoading.value = true;
    const { code, message } = await service.Add_userword({
...
async function modifyExtract() {
    if (!formModel.Sign || !formModelParams.value._id) {
        return
    }
    editBtnLoading.value = true;
    const { code, message } = await service.Modify_userword({
        userName: userName.value,
        userPassword: userPassword.value,
