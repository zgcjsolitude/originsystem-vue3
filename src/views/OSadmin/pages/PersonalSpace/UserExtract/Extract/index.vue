    ...
    <div class="UserExtractTable">
        <div class="OSA-flex-box">
            <div class="flex-left">
                ...
                <el-button class="flex-item" size="small" type="primary" @click="openAddDialog">新增</el-button>
            </div>
        </div>
...
async function getTableData() {
    ...
    const { data, count } = await service.Return_userwordlist({
...
async function deleteColumn(row) {
    if (confirm("此操作不可逆，确认删除？")) {
        const { code, message } = await service.Delete_userword({
            userName: userName.value,
