<template>
    ...
    >   
        ...
        <template #footer>  
            <el-button size="small" type="primary"    @click="formModelParams._action == 'add' ? addFormModel() : modifyFormModel()">
...
async function addFormModel() {
    ...
    const { code, message } = await service.Add_systemextracttype({
...
async function modifyFormModel() {
    ...
    const { code, message } = await service.Modify_systemextracttype({
        userName: userName.value,
        userPassword: userPassword.value,
