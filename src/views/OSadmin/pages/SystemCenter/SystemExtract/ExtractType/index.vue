...
async function getTableData() {
    ...
    store.commit("OsAdmin/vuexLoadSystemExtractTypeList", tableData.value);
}
const columns = [
    {
        align: 'center',
...
async function deleteColumn(row) {
    if (confirm("此操作不可逆，确认删除？")) {
        const { code, message } = await service.Delete_systemextracttype({
            userName: userName.value,
