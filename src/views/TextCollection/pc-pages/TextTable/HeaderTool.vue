<template>
    <div class="d-flex">
        <template v-if="config.classifyType === 'Select'">
            <el-select class="flex-item" v-model="filterParams.classifyPath" :popper-class="storeProfilesgetter" placeholder="请选择类别" clearable @change="selectChange">
            ...
            <el-button class="flex-item" type="primary" @click="openClassifyAdd">
                <el-icon><Plus /></el-icon>
            </el-button>
        </template>
        <template v-if="config.classifyType === 'Tree'">
            <el-button class="flex-item" type="primary" @click="openClassifyDrawer">
                类别筛选
            </el-button>
            <el-input v-model="filterParams.name" class="flex-item" style="width:300px;" placeholder="类别搜索" clearable @clear="clearInput" />
        </template>

        <el-button class="flex-item ml-auto" type="primary" @click="openTableDesigner">
            <el-icon><Grid style="vertical-align: -3px;" /></el-icon>
            表格设计
        </el-button>
        <el-button class="flex-item" type="primary" @click="openArticleDrawer">
            <el-icon><Plus /></el-icon>
            文档新增
        </el-button>
        ...
        <TableDesignerDrawer v-model:visible="tableDesignerDrawer" :appUuid="appUuid" :config="config" @updateTable="updateTable" />
    </div>
</template>

<script setup>
import { onMounted, reactive, ref, watch, inject, toRefs } from 'vue';
import { Edit, Grid, Plus } from '@element-plus/icons-vue';

import ClassifyDrawer from './ClassifyEdit/ClassifyDrawer.vue';
import ClassifyEditDrawer from './ClassifyEdit/ClassifyEditDrawer.vue';
import TableDesignerDrawer from './TableDesigner/EditDrawer.vue';
...
const emit = defineEmits(['updateTableData', 'openArticleDrawer', 'upClassifyData', 'updateTable']);
...
const tableDesignerDrawer = ref(false);
function openTableDesigner() {
    tableDesignerDrawer.value = true;
}
